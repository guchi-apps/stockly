 ...
 * 家庭の境界（#2）を、DBなしで検証する。
 *
 * このテストではDBを使わず、所属の問い合わせをメモリ上のスタブに差し替える。ここで確かめたいのは
 * 「所属していない家庭を指定したときに読み書きの条件を組み立てられないこと」であり、それは
 * `access.ts`だけで決まる。実DB上の複合外部キーが実際に越境INSERTを拒否することは、
 * `db-tests/`（`pnpm test:db`。CIでは`db-constraint-tests`ジョブ）でMySQL/MariaDBに接続して
 * 別途検証する（#18）。
 *
