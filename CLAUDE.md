...
- **ロットの単位は、換算できる単位（同じ次元）の範囲でだけ編集画面から変更できる**（#56）。
  「kgのつもりでgを選んでしまった」のような登録ミスの訂正を想定しており、数量欄の数字は
  入力されたとおり正しい前提で扱う（単位を切り替えても自動では換算しない）。差分は
  現在数量を新しい単位へ換算した値と入力値の差として求め（`operations.ts`の
  `resolveUnitChangeAdjustment()`）、既存の「訂正（ADJUST）」としてそのまま履歴に残す。
  過去の履歴（`InventoryTransaction`）の`unit`は書き換えない——`ledger.ts`の
  `computeLotQuantity()`が各行をロットの現在単位へ都度換算して合算する設計なので、
  換算できる単位間の変更なら整合性は自然に保たれる。次元がまったく違う単位への訂正
  （個数系⇔重さ、等）はこの機能の対象外で、取消して登録し直す運用にする。
  **g⇔kg・mL⇔Lは1000倍の換算なので、換算後の数量が小数4桁以上になることがある**
  （`123.456g`→`0.123456kg`）。`quantity`・`quantityDelta`はDBの`Decimal(14,3)`で
  小数3桁までしか持てないため、`resolveUnitChangeAdjustment()`は収まらない差分を丸めて
  通さず拒否する（丸めて書き込むと、丸めた分だけ集計値と履歴が恒久的にずれる）。
  **`Product.defaultUnit`への書き戻しは、ロットの単位を実際に変えたとき（`params.unit !== lot.unit`）
  だけ行う。** `Product.defaultUnit`そのものと比べると、単位を変えていない編集（名前やカテゴリの
  訂正だけ）でも「別のロットで学習した既定単位」を巻き戻してしまう（計画レビューでの指摘）
- **「使用中」（`StockLot.openedAt`）は商品につき常に高々1つになる**（#57）。登録・編集で
  使用中にチェックすると、同じ商品（`householdId`+`productId`）の他のACTIVEロットの使用中を
  同一トランザクション内で自動的に解除する（`service.ts`の`closeOtherOpenLots()`）。
  排他の単位は保管場所をまたいだ「商品」で、既存データへは遡及しない（今後の保存経路でのみ収束する）。
  **`openedAt`は表示用の値ではなく、防災の除外判定（`disaster/assess.ts`）と写真からの減算候補が
  優先して選ぶロット（`consumption/matching.ts`の`selectLot()`）の両方から読まれる。**
  自動解除で他のロットの`openedAt`が消えると、そのロットに実際は中身が残っていても防災判定では
  「安全な満量」として数えてしまう可能性がある（過大評価側に倒れる）。#57ではこの副作用を
  受け入れる方針で実装した（起票者に確認済み）が、**この列に新しい判定を足すときは、
  「使用中の自動解除」で値が消えうることを前提にする**
