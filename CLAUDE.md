...
- **ロットの単位は、換算できる単位（同じ次元）の範囲でだけ編集画面から変更できる**（#56）。
  ...
  現在数量を新しい単位へ換算した値と入力値の差として求め（`operations.ts`の`convertLotUnit()`）、
  既存の「訂正（ADJUST）」としてそのまま履歴に残す。過去の履歴（`InventoryTransaction`）の
  `unit`は書き換えない——`ledger.ts`の`computeLotQuantity()`が各行をロットの現在単位へ都度
  換算して合算する設計なので、換算できる単位間の変更なら整合性は自然に保たれる。
  次元がまったく違う単位への訂正（個数系⇔重さ、等）はこの機能の対象外で、取消して登録し
  直す運用にする。単位を確定したら`Product.defaultUnit`も書き戻す（カテゴリ・保管場所と
  同じ「最後に確定した値」を覚える考え方）
